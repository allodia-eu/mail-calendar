...
//! sign-off above the signature's, so a last paragraph that repeats the signature's opening, or is
//! made only of its lines, is taken off here.

/// `reply` without a closing paragraph that the signature already carries. A reply of one
/// paragraph is left whole, and so is one when there is no signature.
pub(crate) fn without_closing(reply: &str, signature: Option<&str>) -> String {
    ...
    if repeats_opening || only_signature {
