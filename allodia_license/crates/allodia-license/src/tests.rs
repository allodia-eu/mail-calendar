...
#[test]
fn an_answer_stored_before_payment_status_existed_still_reads() {
    // What an upgrade finds in the host's preferences. A field added since has to default rather
    // than fail the parse: a stored answer that will not read is the free app, and taking a
    // capability away from somebody who paid for it, on an upgrade they may be offline for, is the
    // one direction the grace period exists to avoid.
    let older = r#"{"answer":{"entitlement":{"plan":"personal",
        "capabilities":["Push"],"current_period_end":null},
        "refresh_after_seconds":43200},"fetched_at":1760000000}"#;

    let restored: Stored = serde_json::from_str(older).expect("an older stored answer still reads");

    assert!(restored.answer.entitlement.payment_status.is_none());
    assert!(
        Cache::restore(Some(restored))
            .effective(1_760_000_000 + DAY)
            .grants(&Capability::Push)
    );
}
