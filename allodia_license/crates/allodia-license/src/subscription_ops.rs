...
impl Refusal {
    /// The reason the service gave, when it gave one.
    ///
    /// `None` for a body carrying no refusal envelope at all, which is how a proxy's HTML error
    /// page and a gateway timeout look. **That is not a refusal**: the service never spoke, so
    /// there is nothing it declined, and saying otherwise would tell somebody their subscription
    /// cannot do something when all that happened was a bad minute between here and there.
    ///
    /// A code this version does not recognise *is* a refusal, because the envelope is the
    /// service's own: it said no, in words this build has not learned yet.
    #[must_use]
    pub(crate) fn read(body: &str) -> Option<Self> {
        #[derive(Deserialize)]
        struct Envelope {
            data: Option<Code>,
        }
