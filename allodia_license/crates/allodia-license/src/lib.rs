...
pub struct Entitlement {
    ...
    /// attention.
    ///
    /// Defaulted rather than required, because this type is what the host persists: an answer
    /// stored by a build predating the field has to read back as one that says nothing about
    /// payment, not as a stored answer that cannot be parsed at all, which is the free app for
    /// somebody who was paying.
    #[serde(default)]
