...
/// A refused token is not an answer about the purchase, and it is the one refusal that will refuse
/// every later call in the same pass identically. Folding it into [`LinkOutcome::Deferred`] here
/// would leave a caller unable to tell a revoked grant from a slow afternoon, so it stays an error
/// a caller acts on; what it means for the ledger is still "nothing was learned".
#[test]
fn a_refused_token_is_reported_as_such_rather_than_as_a_quiet_deferral() {
    let service = AccountService::new("https://accounts.example.com");

    for status in [401, 403] {
        let transport = Canned::new(status);
        assert_eq!(
            service
                .link_store_purchase(&transport, "a-stale-token", &purchase("t-1"))
                .expect_err("the token was refused"),
