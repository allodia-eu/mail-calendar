...
/// **A refusal is one the service wrote; an outage is not one.** A proxy's error page carries no
/// envelope, so there is no code, so nothing was declined: reporting it as a refusal would tell
/// somebody their subscription "cannot do that right now" because a load balancer had a moment.
///
/// That is the distinction `entitlement.md` is built on, and these are the four calls where being
/// wrong about it costs money, so it is worth its own test rather than a line in another's.
#[test]
fn a_service_that_never_spoke_is_an_outage_rather_than_a_refusal() {
    let service = AccountService::new("https://accounts.example.com");

    for (status, body) in [
        (502, "<html>Bad Gateway</html>"),
        (500, "<html>maintenance</html>"),
        (504, ""),
        // Shaped like the envelope, but carrying no reason: still nothing declined.
        (
            500,
            r#"{"defined":false,"code":"X","status":500,"message":"x"}"#,
        ),
    ] {
        let transport = Canned::new(status, body);
        assert_eq!(
            service
                .cancel_subscription(&transport, "tok")
                .expect_err("nothing was learned"),
            Error::Unexpected { status },
            "status {status} body {body}"
        );
    }

    // And the one 5xx that *is* an answer: the service says this deployment has no billing.
    let unconfigured = Canned::new(503, &refusal("unavailable"));
    assert_eq!(
        service
            .cancel_subscription(&unconfigured, "tok")
            .expect_err("refused"),
        Error::Refused(Refusal::Unavailable)
    );
}
