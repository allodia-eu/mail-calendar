...
/// The identifiers themselves, written out, because nothing downstream can catch a change to them.
/// Apple never releases a product id for reuse and Play refuses to change one after publication,
/// so what is in the two consoles is permanent and this side has to match it; a store asked for an
/// id it does not know answers with an empty list rather than an error, which draws as a screen
/// with no plans on it and reads as a network problem.
#[test]
fn the_published_identifiers_are_the_ones_the_consoles_hold() {
    assert_eq!(
        Plan::Yearly
            .store_product(Store::Apple)
            .expect("apple sells a product")
            .id
            .as_str(),
        "eu.allodia.mailcal.services.yearly"
    );
    assert_eq!(
        Plan::Monthly
            .store_product(Store::Apple)
            ...
            .id
            .as_str(),
        "eu.allodia.mailcal.services.monthly"
    );
    assert_eq!(
        Plan::Yearly
            .store_product(Store::Google)
