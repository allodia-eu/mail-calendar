...
/// Which store a `source` names.
///
/// ⚠️ **Anything that is not `google` reads as Apple**, and that is a bounded guess rather than a
/// safe default: the service's own enum is closed to those two, so a third value cannot arrive
/// without a change at both ends. If one ever did, this would name somebody's third-party
/// subscription "App Store" beside their money, which is the mistake this contract is otherwise
/// emphatic about.
///
/// It is left this way because the honest alternative is an `Unknown` arm on [`Store`], and that
/// type also answers which product to ask a store for and whether a purchase needs attaching:
/// growing it for a store nobody sells through would put an unreachable arm on both. The day the
/// service adds a third, this is the line that has to change first, and `manage_url` is unaffected
/// either way, because the service sends it rather than this deriving it.
...
pub(crate) fn answered<T: for<'a> Deserialize<'a>>(response: &Response) -> Result<T, Error> {
    match response.status {
        ...
        // **A refusal is one the service wrote, and nothing else is.** What decides is the
        // envelope's own `data.code`, not the status: the service answers `503` with `unavailable`
        // when a deployment has no billing, which is a real answer, and a proxy answers `502` with
        // an HTML page, which is an outage wearing the same digit. Reading the status alone would
        // tell somebody their subscription "cannot do that right now" because a load balancer had
        // a moment, which is the distinction `entitlement.md` is built on, collapsed on the four
        // calls where being wrong costs money.
        status => match crate::Refusal::read(&response.body) {
            Some(refusal) => Err(Error::Refused(refusal)),
            None => Err(Error::Unexpected { status }),
        },
    }
}
