...
impl AccountService {
    ...
    pub fn link_store_purchase(
        ...
        // A refused token is this device's problem rather than an answer about the purchase, and
        // it is the one refusal that will refuse every later call in the same pass identically. It
        // is reported as itself so a caller can say so and stop; the ledger still defers, because
        // nothing was learned and nothing may be finished.
        if let 401 | 403 = response.status {
