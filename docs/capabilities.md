...
| **A forward carries the original's files**: the attachments the reading view lists go out with it, and a forward whose files cannot be read fails rather than going out incomplete. Decided in the core, so every client has it ([docs](docs/sending.md)) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
