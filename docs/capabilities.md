...
| **Setting a server up by hand**: the server name, its **port** and its connection security (SSL/TLS or STARTTLS) side by side, for a mailbox autodetection cannot find. The port follows the security you pick until you type one of your own, and then it is yours ([docs](docs/account-autodetect.md)) | ✅ | 🚧 | 🚧 | ✅ | 🚧 | ✅ |
| **Mail software on your own machine**: a server whose certificate no recognised authority vouches for (Proton Mail Bridge, a self-hosted server) is shown to you, fingerprint included, and connects only once you accept that one certificate for that one server; the acceptance is kept with the account, so you are asked once. Verification is never switched off ([docs](docs/certificate-exceptions.md)) | ✅ | ✅ | ✅ | ✅ | 🚧 | 🚧 |
...
| **Your subscription**, under the same Settings category: what is being charged and by whom, when it renews or runs out, a failed payment the store is still retrying (which stops nothing), a warning naming every biller when more than one is charging for one account, and a link to the store's own page, which is the only place a store's subscription can be changed. Where a subscription can be bought, both periods are offered at the store's own price, and where Allodia bills you directly you can also cancel, restart or move between monthly and yearly ([docs](allodia_license/purchasing.md)) | ✅ | 🚧 | 🚧 | ✅ | ✅ | ✅ |
...
| A message written for a wider pane (a fixed 600px newsletter) is **reflowed to fit** rather than cut off at the right edge, at every pane width and live as a window is resized; what reflow cannot narrow, Android also scales down ([docs](docs/reading-zoom.md)) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Pinch to zoom a message**, touchscreen or trackpad, 0.25×–5× and 1×–5× on iPhone and iPad, starting again at each message's own scale; Windows adds Ctrl+scroll and Ctrl +/−, Linux Ctrl+scroll ([docs](docs/reading-zoom.md)) | — | ✅ | ✅ | ✅ | ✅ | ✅ |
| The reading header (who sent it, who else got it, what it carries) **scrolls away with the message**, leaving the action row in place ([docs](docs/reading-actions.md)) | — | — | ✅ | — | — | — |
...
| **The message you are writing is kept on the server**: the composer stores it in Drafts once you stop typing for half a minute, and Save as draft stores it at once. It is the same draft each time rather than a new one, so your other devices and your webmail see one message being written, and sending it takes the stored copy away ([`docs/drafts.md`](docs/drafts.md)) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **A draft opens back into the composer**: a row in the Drafts folder opens what you were writing, holding its files, and saves over that copy rather than beside it. Closing the composer leaves the draft where it is; only Discard takes it off the server | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **A message in a window of its own**: double-click a row (or "Open in new window" on its context menu) and it opens as a separate window carrying the whole reading view and its action row, so several messages can be read side by side while the mailbox stays where it was. Replying or forwarding from one opens the draft in its own window. Closing the mailbox closes them all, and however many are open there is still one app on one database ([docs](docs/reading-window.md)) | ✅ | ✅ | — | ✅ | — | ✅ |
...
| **Paste a picture into the message**, or into a signature you are editing: a screenshot on the clipboard, or a picture file you copied, lands where you are typing and is sent as an inline `cid:` part, not an attachment ([docs](docs/composer-security.md) Gate 13) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Resize a picture in the message**: drag a corner of a pasted or quoted picture to size it, with a mouse, a trackpad, a pen or a finger; the ratio is kept and the width goes out with the message ([docs](docs/composer-security.md) Gate 13) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
| Open `mailto:` links: the OS offers Allodia Mail & Calendar as a mail app, and a tapped mail link opens the composer pre-filled (To/Cc/Bcc/Subject/Body, decoded in the core; every other header a link may name is dropped; [`docs/composer-security.md`](docs/composer-security.md) Gate 12) | ✅ | ✅ | 🚧 (registered; iOS routes `mailto:` only to the **default** mail app, which needs Apple's `com.apple.developer.mail-client` entitlement) | ✅ | ✅ | ✅ |
| **Share a file into a new message**: the OS's own share sheet (or "Open With") offers Allodia Mail & Calendar, and a composer opens with the files already attached. The core names and types every shared file, drops what could break a header or disguise an extension, and reports back anything it would not attach. On macOS the sharing extension is switched on in System Settings ▸ General ▸ Login Items & Extensions ▸ Sharing, as every third-party one is ([`docs/os-integration.md`](docs/os-integration.md)) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Offer to become your default mail app**: asked once, never before an account exists and never where the build cannot act on it, with a permanent Settings → General entry to change your mind. Not offered on Linux or Android, where nothing in the platform can be asked, nor in the sandboxed Mac App Store build, which the sandbox refuses ([`docs/os-integration.md`](docs/os-integration.md)) | ✅ | ✅ (Developer ID) | ⬜ | ✅ | — | — |
| **Know whether you are running the old one**: Settings → About says what keeps this copy up to date, naming the mechanism that actually holds its updates rather than claiming in general that the app updates itself, and checks on request. A check that could not get through says so, instead of reporting that you are current ([`docs/updates.md`](docs/updates.md)) | — | ⬜ | ⬜ | ✅ | ⬜ | ⬜ |
...
| Calendar: **setting a repeating event's rule**: how often, how many periods to skip, which weekdays it falls on, and what ends it, or stopping the repeat altogether. A rule too detailed to state in full is shown and not offered, so an editor never writes back a partial picture ([`docs/calendar.md`](docs/calendar.md)) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
| Contacts: a unified A–Z list of people from every account's address books (CardDAV · JMAP · Google), with search and a detail view ([`docs/contacts.md`](docs/contacts.md)) | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
