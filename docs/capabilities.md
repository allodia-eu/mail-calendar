...
| Mail accounts (IMAP/SMTP) **sign in with your provider** where the server offers it, discovered from the standards with no per-provider code; the setup screen asks the server what it accepts before drawing a credential field, and says so when a provider's sign-in exists but admits only pre-registered applications ([`docs/mail-oauth.md`](docs/mail-oauth.md)) | ✅ | 🚧 | 🚧 | ⬜ | ✅ | ✅ |
