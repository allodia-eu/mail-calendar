...
- **Sharing *into* the app (Gate 15) ships on every platform.** `prefill_from_share` decodes a
  share, names and types its files and reports what it refused, with suites in `mailcal-composer`,
  `mailcal-bindings`, the Linux crate, the Android JVM suite, `Mailcal.Tests` and `MailcalKitTests`.
  Only the Apple clients put the refusals on screen, though; the other three count them into the
  log and stop, which [`os-integration.md`](os-integration.md) records under its own known gaps.
