...
| Shared files named and typed by the core (Gate 15) | ⬜ no share target registered | ⬜ no `ACTION_SEND` filter (`MailtoLaunchTest` asserts one is *not* treated as a mail link) | ⬜ no `windows.shareTarget` extension | ⬜ no file `MimeType=` in the desktop entry |
...
- **Sharing *into* the app has its gate (Gate 15) and its core, and no client yet.**
