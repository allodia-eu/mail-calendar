...
| Shared files named and typed by the core (Gate 15) | ⬜ no share target registered | ⬜ no `ACTION_SEND` filter yet (`MailtoLaunchTest` asserts one is not a *mail link*, which stays true) | ⬜ no `windows.shareTarget` extension | desktop `MimeType=` + `%U` and `--attach` → `prefill_from_share`; the composer opens holding `ComposeRequest::files` |
...
- **Sharing *into* the app (Gate 15) ships on Linux only.** `prefill_from_share` decodes a share,
