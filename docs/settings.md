...
- **Notifications is on every platform.** Each posts through its own host: macOS and iOS/iPadOS
  through `UNUserNotificationCenter`, Windows through the Windows App SDK's
  `AppNotificationManager`, Android through `NotificationCompat` and Linux through the desktop
  portal ([`background-sync.md`](background-sync.md)). Android alone adds a second item here, the
  battery-exemption card, and only while it is not yet exempt.
...
- **Notification *content* is still English-only on Android.** The settings copy is catalog-backed
  on Android and iOS, and both settings and notification copy are catalog-backed on macOS, Linux
  and Windows. The Android notification channel name and notification texts ("N new messages",
  "+N more") remain hardcoded: they render in a background context where the per-app locale needs
  separate handling ([`background-sync.md`](background-sync.md) known gap).
