...
- **macOS omits Notifications.** Windows, Android, iOS/iPadOS and Linux show it; Linux posts
  through the desktop portal and Windows through the Windows App SDK's `AppNotificationManager`.
  macOS leaves the slot out, blocked on its notification host adapter
  ([`background-sync.md`](background-sync.md)); the category keeps its position for when it gains
  one.
...
- **Notification *content* is still English-only on Android.** The settings copy is catalog-backed
  on Android and iOS, and both settings and notification copy are catalog-backed on Linux and
  Windows. The Android notification channel name and notification texts ("N new messages",
