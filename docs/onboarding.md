...
- **This screen's store capture is wired on all five and seen on one.** The capture named
  ...
  Every client now asks for that boot on that screen, and Windows is the only one it has been run
  on: Apple, Android and Linux are written against the same seam and were not built on the host
  this was written on. The four `setup-*` documentation captures are untouched by it, being a later
  add by design.
