...
- **This screen's store capture is wired on all five and run on all five.** The capture named
  ...
  Every client asks for that boot on that screen, and every one has been run. The four `setup-*`
  documentation captures are untouched by it, being a later add by design.
