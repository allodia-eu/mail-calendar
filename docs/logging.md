...
`cargo xtask check-log-hygiene` still fails one, because it inspects string literals and a crash
location is never a literal.

The narrow, unambiguous half of this is machine-enforced by
[`cargo xtask check-log-hygiene`](../xtask/src/log_hygiene.rs) (in the gate): a repo path, a `.md`
