...
| [`list-selection.md`](list-selection.md) | The cross-platform contract for selecting several messages and acting on the lot at once: the selection is the client's (its list control already has the modifier and assistive-technology behaviour) while the batch is the core's one intent, a conversation row stands for its whole thread and a move never takes a Sent copy out of Sent, the selection is scoped to the list on screen, and the desktop keys are bound on the list rather than app-wide. |
