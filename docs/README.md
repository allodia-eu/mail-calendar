...
| [`os-integration.md`](os-integration.md) | The two doors the OS knocks on and the question that decides whether the first is ever used: `mailto:` registration, sharing a file into a new message, and offering to become the default mail app. Which entry point each client registers, what the core decides about a shared file's name and media type, the rule that a share may never address a message, the standing prohibition on `mailto:?attach=`, and what each platform can and cannot be asked (Apple needs an entitlement, Windows can only deep-link, Linux and Android cannot be asked at all). |
...
| [`capabilities.md`](capabilities.md) | What each client ships, capability by capability. The at-a-glance truth the whole product is measured against: a store listing, a release note or a README sentence may never claim more than a row here does. Moved out of the README so that document stays readable. |
...
| [`store-listing.md`](store-listing.md) | The cross-platform contract for app-store copy: the stores' field limits, the rule that one description body is written once and reused verbatim across every store, how the Linux metainfo is generated, and the two hard couplings: copy may not out-run the capability matrix, and every privacy claim must match `privacy-policy.md`. The copy itself lives beside the brand it belongs to, in `branding/<brand>-listing.md`. |
| [`changelog.md`](changelog.md) | The cross-platform contract for release notes ("What's new"): a change writes one **fragment** under `changelog/unreleased/` (per-locale notes, the `Platforms:` it reached, `minor` or `patch`), and `scripts/dev/release.py` assembles the pending ones into `changelog/released/X.Y.Z.md` and moves `/VERSION`. One file per change, so two PRs never conflict; each note is measured against the tightest store its own platforms reach, with the same anti-hype (matches the capability matrix) and privacy-consistency couplings as the store listing. |
