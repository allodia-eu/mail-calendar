...
| [`drafts.md`](docs/drafts.md) | Keeping an unfinished message on the server. The stored copy is named by the **key the provider answered with**, never by anything the message carries, because three of the four transports move that key on a re-save and the fourth moves the `Message-ID`. A save is the one mail write this app repeats on purpose, so it supersedes rather than adds; an unchanged draft costs no write; two saves of one composition never overlap; and a discard withdraws a save still queued. Resuming one opens a composer that saves **over** it, so it opens holding the draft's files or not at all; an accepted send takes the stored copy away and a failed one leaves it. |
...
| [`certificate-exceptions.md`](docs/certificate-exceptions.md) | Reaching a server whose certificate does not verify, without any client turning verification off. An exception is consulted only **after** the policy has refused, and pins **one** certificate on **one** server name, so it is not "stop checking this host". Nothing is accepted that was not shown, fingerprint included; Connect stays inert until it is; and the acceptance is stored with the account, so nobody is asked twice. |
...
| [`os-integration.md`](docs/os-integration.md) | The two doors the OS knocks on: a `mailto:` link and a share. Both pre-fill an **editable** composer and never send. A share may suggest a subject and body and **never** recipients; the one route to `To`/`Cc`/`Bcc` is shared text that is itself a mail link, decoded by the same allowlist. **`mailto:?attach=` is never honoured**: a handler cannot tell a page's URI from a local one, so files arrive only through a channel that is itself a user action. Names and media types are normalised in the core, and a refused file is reported, never dropped silently. The offer to become the default mail app is put **once**, never before an account exists, and never where the build cannot act. |
| [`updates.md`](docs/updates.md) | Settings → About says **what keeps this copy current**, named as the thing a user would go to, and answers a check with one of three outcomes. The mechanism is read from the platform, never from the brand or the build. A check that failed is **never** reported as up to date. A client never claims to have installed what the platform will apply at the next launch. None of it belongs in the core: there is no shared state, only a different OS API per platform. |
| [`windows-channels.md`](docs/windows-channels.md) | Windows ships the same build twice, to the Store and to a download we host. They are two **packages**, because a package outside the Store carries the subject of the certificate that signed it and the Store's publisher is a GUID only Microsoft can sign for. The direct channel publishes the framework package the Store would have resolved, refuses an unsigned bundle, and keeps its `.appinstaller` URL permanent because that is where an installation looks for updates. Both installed at once is two writers on one mail store, and only copy holds that line. |
...
| [`purchasing.md`](allodia_license/purchasing.md) | How an account becomes paid: one service under two periods, sold by Allodia's own checkout and by the two phone stores, which add their commission on top. Prices are the shop's own string, never written or computed here. A purchase is **never finished with the store until the account service has granted it**, because StoreKit re-delivers an unfinished transaction and Play refunds an unacknowledged one after three days. The device carries the store's proof without opening it, and the account is the access token's, never the body's. |
...
| Paid capabilities and the Allodia account | [`allodia_license/`](allodia_license), governed by [`allodia_license/entitlement.md`](allodia_license/entitlement.md) and [`allodia_license/purchasing.md`](allodia_license/purchasing.md) |
| Machine-specific build tuning | `AGENTS.local.md`, untracked; one developer's disk and toolchain are not a repo fact |
| The store pushes, the brand assets and the release runbooks | Not here. They belong to whoever publishes this app under their own name, and each push reads this tree through a checkout. What is here is the contract they obey ([`docs/store-listing.md`](docs/store-listing.md)), the readers they share ([`scripts/dev/store_payload.py`](scripts/dev/store_payload.py)), the gate that measures the copy ([`scripts/ci/check_store_copy_length.py`](scripts/ci/check_store_copy_length.py)) and the captures they upload ([`scripts/dev/showcase.sh`](scripts/dev/showcase.sh)) |
