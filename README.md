...
This repository is the app's **core**. Deciding what happens is Rust's job here and drawing it is
the client's: the product logic, the presentation state machines every client renders, the
host-service ports each platform implements and the UniFFI/C-ABI bindings all live in this tree.
Beneath it sits the product-neutral PIM sync engine, which is its own repository; above it, the four
native clients under [`clients/`](clients): one for Apple's three platforms, one each for Windows,
Android and Linux. So a rule is written once here rather than five times, and cannot drift between
the clients that render it.
...
There is no Linux release; the matrix below says why. Building any of them from this source needs no
credential of ours.
...
## Status: platform capability matrix
