// The learn sheet's state (docs/ai.md, "Learning"): which account, which sent mail, and the report
// the consent is given against. WinUI-free so Mailcal.Tests can pin the two rules that are silent
// when wrong: a report that arrives after the person changed the account or the range must not
// stand for the new choice, since consent is given against what it says; and "up to a date" takes
// in the whole of that day, in the zone the app shows dates in.
...
internal sealed class LearnSheet
{
    ...
    internal LearnSheet(string account, DateOnly today)
    {
        Account = account;
        UntilDay = today;
    }
