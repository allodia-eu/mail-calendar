...
//
// Both slide sideways with the grid (they share its `dayOffset`), so a column and its heading can
// never drift apart.
...
struct CalendarDayHeader: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 0) {
            ...
            .frame(width: weekWidth, alignment: .leading)
            .offset(x: -dayOffset)
...
struct CalendarAllDayBanner: View {
    ...
    var body: some View {
        let lanes = Int(page.allDayLanes)
        ...
        let hidden = allDayOverflowPerDay(
            bands: page.allDay, dayCount: dayCount, drawnLanes: drawnLanes
        )
        ...
        HStack(alignment: .top, spacing: 0) {
            ...
            ZStack(alignment: .topLeading) {
                ForEach(page.allDay.filter { Int($0.lane) < drawnLanes }, id: \.rowID) { band in
                    CalendarAllDayChip(
                        band: band, calendars: page.calendars, dayWidth: dayWidth, dark: dark,
                        onOpen: {
                            onOpen(
                                EventRefID(
